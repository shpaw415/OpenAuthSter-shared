import { and, drizzle, eq } from "../database/drizzle";
...
export class WebHook {
	...
	>({
		...
		> = await Promise.all(
			webhooks.map(this.parseWebHookConfig).map(async (webhook) => {
				try {
					const fullPayload: WebHookPayLoad<(typeof webhook)["event"]> = {
						id: webhook.id,
	...
	static async getWebHookPayloadFromRequest<AwaitedEvent extends WebHookEvents>(
		...
		>
	> {
		const payload = await WebHook.getPayLoadFromRequest(request);
		await WebHook.ensureAuthenticity({
			request,
			secret: appSecret,
			data: payload,
		});
		...
		WebHook.ensureTimeStamp(parsedPayload);
